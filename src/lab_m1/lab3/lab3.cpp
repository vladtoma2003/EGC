...
void Lab3::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh2D(meshes["car"], shaders["VertexColor"], modelMatrix);

    modelMatrix = glm::mat3(1);
    modelMatrix *= transform2D::Translate(carX-0.5*carSide, carY-0.5*carSide);
    
    RenderMesh2D(meshes["rectangle"], shaders["VertexColor"], modelMatrix * transform2D::Scale(3, 1));

    modelMatrix = glm::mat3(1);
    modelMatrix *= transform2D::Translate(carX-0.25*carSide, carY-0.75*carSide);
    modelMatrix *= transform2D::Scale(0.5, 0.5);
    modelMatrix *= transform2D::Rotate(2*angularStep*rotate);
    modelMatrix *= transform2D::Translate(-0.5*carSide, -0.5*carSide);
    
    RenderMesh2D(meshes["tire"], shaders["VertexColor"], modelMatrix);

    modelMatrix = glm::mat3(1);
    modelMatrix *= transform2D::Translate(carX+1.75*carSide, carY-0.75*carSide);
    modelMatrix *= transform2D::Scale(0.5, 0.5);
    modelMatrix *= transform2D::Rotate(2*angularStep*rotate);
    modelMatrix *= transform2D::Translate(-0.5*carSide, -0.5*carSide);
